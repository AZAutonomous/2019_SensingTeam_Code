...
for imagesFold,labelsFold in zip(myImagesInFolds,myLabelsInFolds):

    # Splits the data in train and test data
    trainX, testX = np.split(imagesFold, [int(0.9 * len(imagesFold))])
    trainY, testY = np.split(labelsFold, [int(0.9 * len(labelsFold))])
